...
function PRApproval() {
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      const getPurchaseReturns = async () => {
        const url = `${api}/get_pending_purchaseReturn`;
        const result = await GET(token, url);
      ...
      const updatedPurchaseReturns = purchaseReturns?.map((pr) => {
        const supplierName =
          vendors?.find((vendor) => vendor.id === pr.supplier_id)?.supplier_name || "";
        const warehouseName =
          warehouses?.find((warehouse) => warehouse.id === pr.warehouse_id)?.warehouse_name || "";
  ...
  const column = useMemo(
    () => [
      ...
      {
        field: "date_of_pr", headerName: "Date Of PR", width: 150,
      },
      { field: "no_of_products", headerName: "Total no. of products return", width: 120 },
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          <button
            ...
            onClick={() => {
              navigate(
                `/editpurchaseReturn/${params.row.id}/?isFrom=approval`
              );
            }}
          >
      ...
      },
      {
        ...
        renderCell: (params) => (
          <button
            ...
            onClick={() => {
              const pdfUrl = `/purchasePDFViewer/${params.row.id}/?isFrom=prApproval`;
  ...
  const handleConfirm = async () => {
    ...
    const url = `${api}/update_purchaseReturn_status`;
    setUpdatedState(false);
    ...
    if (update.response === 200) {
      ...
      setalertMsg(`PR ${actionType === "approve" ? "approved" : "rejected"} succesfully.`);
  ...
  return (
    <div style={{ height: "100%" }}>
              ...
              onChange={(e) => {
                ...
                setTimeout(() => {
                  function searchArrayByValue(arr, searchQuery) {
                    const keysToSearch = ["pr_no", "pi_no", "supplier_name", "warehouse_name", "city", "date_of_pr" , "no_of_products", "no_of_qty", "pi_amount", "total_amount", "pr_status"];
                  ...
                  setPurchaseReturns(
                    searchArrayByValue(allPurchaseReturns, e.target.value.toLowerCase())
                  );
                }, 500);               
              }}
            />
      ...
      >
        ...
        >
          <Typography>{`Are you sure you want to ${actionType === "approve" ? "Approve" : "Reject"
            } this PR?`}</Typography>
          <div
            style={{
              display: "flex",
