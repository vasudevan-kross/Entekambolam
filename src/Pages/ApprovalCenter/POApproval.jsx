...
function POApproval() {
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      const getPurchaseOrders = async () => {
        const url = `${api}/get_pending_purchaseOrder`;
        const result = await GET(token, url);
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          <button
            ...
            onClick={() => {
              navigate(
                `/editpurchaseorder/${params.row.id}/?isFrom=approval`
              );
            }}
          >
      ...
      },
      {
        ...
        renderCell: (params) => (
          <button
            ...
            onClick={() => {
              const pdfUrl = `/purchasePDFViewer/${params.row.id}/?isFrom=poApproval`;
  ...
  const handleConfirm = async () => {
    ...
    const url = `${api}/update_purchaseOrder_status`;
    setUpdatedState(false);
    ...
    if (update.response === 200) {
      ...
      setalertMsg(`PO ${actionType === "approve" ? "approved" : "rejected"} succesfully.`);
              ...
              onChange={(e) => {
                ...
                setTimeout(() => {
                  function searchArrayByValue(arr, searchQuery) {
                    const keysToSearch = ["po_no", "supplier_name", "warehouse_name", "city", "date_of_po", "date_of_delivery", "delivery_time", "no_of_products", "no_of_qty", "total_amount", "po_status"];
                  ...
                  setPurchaseOrders(
                    searchArrayByValue(allPurchaseOrders, e.target.value.toLowerCase())
                  );
                }, 500);
              }}
            />
